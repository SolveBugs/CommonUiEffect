...
import com.example.zq.commonuieffect.tools.BaseTools;

/**
 * Created by zhenqiang on 2017/1/5.
 */
...
public class CurtainView extends RelativeLayout implements View.OnTouchListener {
    ...
    private RelativeLayout can_scroll;
    ...
    private void init(Context context) {
        ...
        final View view = LayoutInflater.from(mContext).inflate(R.layout.curtain_view_layout, this, false);
        ...
        can_scroll = (RelativeLayout) view.findViewById(R.id.can_scroll);
        addView(view);
...
//                CurtainView.this.scrollTo(0, curtainHeigh);
                can_scroll.scrollTo(0, curtainHeigh);
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            can_scroll.scrollTo(mScroller.getCurrX(), mScroller.getCurrY());
            //更新界面
            postInvalidate();
            isMove = true;
        } else {
                    ...
                    if (scrollY < 0) {
                        ...
                        if (!isOpen) {
                            if (scrollY <= curtainHeigh) {
                                can_scroll.scrollTo(0, curtainHeigh - scrollY);
                            }
                        }
                    }
                    break;
                    ...
                    if (downY > upY) {
                        ...
                        if (isOpen) {
                            if (Math.abs(scrollY) > curtainHeigh / 2) {
                                ...
                                startMoveAnim(can_scroll.getScrollY(),
                                        (curtainHeigh - can_scroll.getScrollY()), upDuration);
                                isOpen = false;
                            } else {
                                startMoveAnim(can_scroll.getScrollY(), -can_scroll.getScrollY(), upDuration);
                                isOpen = true;
                            }
                        }
                    } else {
                        ...
                        if (scrollY > curtainHeigh / 2) {
                            ...
                            startMoveAnim(can_scroll.getScrollY(), (curtainHeigh - can_scroll.getScrollY()), upDuration);
                            isOpen = false;
                        }
                    }
                    break;
